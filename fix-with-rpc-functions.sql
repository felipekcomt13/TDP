...
BEGIN
  ...
  -- Verificar permisos: debe ser admin o dueño de la reserva
  IF NOT es_admin_usuario AND reserva_actual.user_id != usuario_actual THEN
    RAISE EXCEPTION 'No tienes permiso para confirmar esta reserva. Solo los admins o el dueño pueden confirmar reservas';
  END IF;
...
'Confirma una reserva. Solo admins o el dueño de la reserva pueden confirmar.';
...
📋 FUNCIONES CREADAS:
  • confirmar_reserva(reserva_id uuid) → Confirma una reserva
  • rechazar_reserva(reserva_id uuid) → Rechaza una reserva

🔒 SEGURIDAD:
  • Usan SECURITY DEFINER para bypassear RLS
  • Verifican permisos dentro de la función
  • Solo admins pueden rechazar
  • Admins o dueños pueden confirmar
